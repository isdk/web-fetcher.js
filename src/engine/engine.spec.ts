...
const engineTestSuite = (
  ...
  describe.sequential(`FetchEngine Suite: ${engineName}`, () => {
    ...
    it(
      'should goto a page and get content',
      async () => {
    ...
    it(
      'should handle page navigation by clicking a link',
      async () => {
        await engine.goto(baseUrl)
    ...
    it(
      'should fill and submit a form as JSON',
      async () => {
        await engine.goto(baseUrl)
        await engine.fill('input[name="test_input"]', 'new_value')
        await engine.submit('form[action="/submit"]', {
    ...
    it(
      'should fill and submit a form as urlencoded',
      async () => {
        await engine.goto(baseUrl)
        await engine.fill(
          'input[name="urlencoded_input"]',
          'new_urlencoded_value'
        )
        await engine.submit('form[action="/submit_urlencoded"]') // Default enctype
        const content = await engine.getContent()
        expect(content.text).toContain(
          'Submitted urlencoded: new_urlencoded_value'
        )
      },
      TEST_TIMEOUT
    )
    ...
    it(
      'should wait for elements or time',
      async () => {
    ...
    it(
      'should extract structured data from a page',
      async () => {
        const testSchema = {
          type: 'object',
          properties: {
    ...
    it(
      'should handle advanced extraction cases with missing selectors and contexts',
      async () => {
        await engine.goto(`${baseUrl}/extract-test`)
    ...
    it(
      'should handle advanced extraction with shorthands, has, and exclude',
      async () => {
        ...
        expect(excludeDescendantData).toEqual([
          '/link1',
          '/link2',
          '/link3',
          '/link5',
        ...
        expect(textShorthandData).toEqual([
          'Item 1 Text',
          'Item 2 Text',
          'Item 3 Text',
        ])
    ...
    if (engineName === 'playwright') {
      it('should not use firefox by default when antibot is disabled', async () => {
        ...
        const controlContext: FetchEngineContext = {
        ...
        let content = await controlEngine.extract<string>({
          selector: '#results',
        })
