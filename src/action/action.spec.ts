...
const actionTestSuite = (
  ...
  describe.sequential(`FetchAction Suite: ${engineName}`, () => {
    ...
    beforeEach(async () => {
      ...
      const engineCtx: FetchEngineContext = {
        id: `test-action-${engineName}-${Date.now()}`,
    ...
    it(
      'should execute getContent action',
      async () => {
    ...
    it(
      'should execute click action to navigate',
      async () => {
        ...
        await clickAction!.execute(context, {
          params: { selector: 'a[href="/page2"]' },
        })
    ...
    it(
      'should execute fill and submit actions',
      async () => {
        await engine.goto(baseUrl)
    ...
    it(
      ...
      async () => {
        ...
        await engine.goto(baseUrl)
        const result = await waitForAction!.execute(context, {
          params: { ms: 500 },
        })
    ...
    it(
      ...
      async () => {
        ...
        const result = await extractAction!.execute(context, {
          params: testSchema,
        })
    ...
    it(
      'should execute pause action',
      async () => {
        let pauseCalled = false
        let pauseMessage = ''

        // Add onPause to the engine's context.
        ;(engine.context as any).onPause = async (params: {
          message?: string
        }) => {
          pauseCalled = true
        ...
        const result = await pauseAction!.execute(context, {
          params: { message: 'Action pause test' },
        })
