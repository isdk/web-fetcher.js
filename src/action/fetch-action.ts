...
import { FetchReturnType, FetchReturnTypeFor } from "../core/fetch-return";
...
export abstract class FetchAction {
  ...
  static capabilities: FetchActionCapabilities = { http: 'noop', browser: 'noop' };
  ...
  protected installCollectors(context: FetchContext, options?: FetchActionOptions) {
    ...
    for (const raw of configs) {
      ...
      const doBefore = async (payload?: any) => {
        ...
        started = true;
        try { await collector.onBeforeExec?.(context, raw as any); }
        catch (e) { context.eventBus.emit('collector:error', { action: this.id, collector: collector.id, phase: 'before', error: e }); }
      };
      ...
      const doEach = async (eventName: string | RegExp, payload?: any) => {
        ...
        try {
          const p = Promise.resolve(collector.onExecute?.(context, raw as any, payload))
            .then(res => {
              if ((raw as any).storeAs) {
                const arr = (context.outputs[(raw as any).storeAs!] ||= []);
                arr.push(res);
              }
              context.eventBus.emit('collector:result', { action: this.id, collector: raw.id || raw.name, event: eventName, result: res });
              return res;
            })
            .catch(error => {
              context.eventBus.emit('collector:error', { action: this.id, collector: raw.id || raw.name, event: eventName, phase: 'exec', error });
            })
            .finally(() => {
              execCount++;
            });
          ...
          context.eventBus.emit('collector:error', { action: this.id, collector: collector.id, event: eventName, phase: 'exec', error });
        }
      };
      ...
      const doAfter = async () => {
        ...
        ended = true;
        try { await collector.onAfterExec?.(context, raw as any); }
        catch (e) {
          context.eventBus.emit('collector:error', { action: this.id, collector: raw.id || raw.name, phase: 'after', error: e });
        } finally {
          context.eventBus.emit('collector:end', { action: this.id, collector: raw.id || raw.name });
      ...
      if (!activateOn.length && !collectOn.length && !deactivateOn.length) {
        const endHandler = () => { doAfter(); };
        context.eventBus.once(`action:${this.id}.end`, endHandler);
        unsubs.push(() => context.eventBus.off('fetcher:action:end', endHandler));
      }
    }
    if (unsubs.length || allPendings.size > 0) {
      return {
        ...
        awaitExecPendings: async () => {
          if (allPendings.size > 0) await Promise.allSettled(Array.from(allPendings));
        },
 ...
 // ============ 生命周期方法（基类实现，子类可覆盖）============
  ...
  async afterExec(
    ...
    try {
      ...
      try { context.eventBus.emit(`action:${this.id}.end`, data); } catch (e) {}
      try { context.eventBus.emit('action:end', data); } catch (e) {}
    } finally {
      try { collectors?.cleanup(); }
      finally {
        stack.pop();
...
type CollectorsRuntime = { cleanup: () => void; awaitExecPendings: () => Promise<void> };

function arrify<T>(v?: T | T[]): T[] { if (!v) return []; return Array.isArray(v) ? v : [v]; }

function subscribeOnce(ctx: FetchContext, patterns: Array<string | RegExp>, fn: (ev?: any) => void) {
  const unsubs: Array<() => void> = [];
  for (const p of patterns) {
    if (typeof p === 'string' || p instanceof RegExp) {
      const h = (...args: any[]) => { fn(args[0]); };
...
function subscribeEach(ctx: FetchContext, patterns: Array<string | RegExp>, fn: (evName: string | RegExp, payload: any) => void) {
  const unsubs: Array<() => void> = [];
  for (const p of patterns) {
    if (typeof p === 'string' || p instanceof RegExp) {
