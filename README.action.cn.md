...
* **`anchor`** (string, optional): 指定该字段提取的起始锚点。
    * **字段名**: 引用同一对象中先前已提取的某个字段对应的 DOM 元素作为锚点。
    * **CSS 选择器**: 在当前对象的根作用域内重新查找该选择器，将其结果作为锚点。
    * 锚点生效后，该字段的搜索范围将变为锚点**之后**的兄弟节点（类似于 `relativeTo: "previous"`，但拥有显式的起点）。
    * **自动冒泡**: 如果锚点位于深层嵌套结构中，引擎会自动向上“冒泡”以找到当前作用域内的有效容器。
