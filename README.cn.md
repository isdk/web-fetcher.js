...
* `fill`: 向输入框填充指定的值。
* `submit`: 提交一个表单。
* `trim`: 从 DOM 中移除元素以清理页面（例如：脚本、广告、隐藏内容）。
* `waitFor`: 暂停执行，以等待一个特定的条件（例如：超时、选择器出现或网络空闲）。
* `pause`: 暂停执行以进行手动干预（例如，解决验证码）。
* `getContent`: 获取当前页面状态的完整内容（HTML、文本等）。
* `extract`: 使用富有表现力的声明式 Schema, 可轻松提取页面中的任意结构化数据。支持 `required` 必填字段和 `strict` 严格校验，确保采集质量。
